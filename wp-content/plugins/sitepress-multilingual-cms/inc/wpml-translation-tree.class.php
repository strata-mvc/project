...
class WPML_Translation_Tree {
		...
		foreach ( $this->language_order as $lang ) {
			...
			if ( isset( $terms_in_trids[ $lang ] ) ) {
				...
				foreach ( $terms_in_lang_and_trids as $trid => $terms_in_lang_and_trid ) {
					...
					$term_names [ $trid ] = $term_in_lang_and_trid[ 'name' ];
				}

				natsort ( $term_names );

				foreach ( $term_names as $trid => $name ) {
					foreach ( $terms_in_trids as $language_code => &$terms_in_lang ) {
						unset( $terms_in_lang[ $trid ] );
					}
					$ordered_trids [ ] = $trid_groups[ $trid ];
				}
			}
		}
	...
	public function sync_tree ( $lang ) {
		...
		if ( $sitepress->get_option ( 'sync_taxonomy_parents' ) ) {
