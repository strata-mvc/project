...
class pb_backupbuddy_backup {
	...
	function process_backup( $serial, $trigger = 'manual' ) {
		...
		if ( $response === false ) { // Function finished but reported failure.
			...
			if ( 'backup_create_database_dump' != $this->_backup['steps'][$step_index]['function'] ) { // Wipe arguments except for DB backup which tracks tables dumps. Keeps fileoptions for growing crazily for finished steps containing state data such as deployment or new zip functionality passing chunking state.
