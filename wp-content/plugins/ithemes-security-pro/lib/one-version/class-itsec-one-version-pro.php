...
class ITSEC_One_Version_Pro {

	function __construct() {

		if ( is_admin() && get_site_option( 'itsec_pro_just_activated' ) && ( ! isset( $_SERVER['HTTP_REFERER'] ) || ( strpos( sanitize_text_field( $_SERVER['HTTP_REFERER'] ), 'action=upload-plugin' ) === false && strpos( sanitize_text_field( $_SERVER['HTTP_REFERER'] ), 'action=install-plugin' ) === false ) ) ) {
