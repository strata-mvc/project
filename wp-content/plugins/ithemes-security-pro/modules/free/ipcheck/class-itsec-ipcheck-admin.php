...
class ITSEC_IPCheck_Admin {
		...
		if ( isset( get_current_screen()->id ) && ( strpos( get_current_screen()->id, 'security_page_toplevel_page_itsec_settings' ) !== false ) ) {
			...
			wp_localize_script(
				...
				array(
					...
					'text1'     => __( 'Receive email updates from iThemes', 'it-l10n-ithemes-security-pro' ),
					'text2'     => __( 'Leverage the power of the iThemes Brute Force Protection network to ban IPs hitting your site.', 'it-l10n-ithemes-security-pro' ),
					'text3'     => __( 'Enter email and click Save All Changes', 'it-l10n-ithemes-security-pro' ),
				)
			);
	...
	public function itsec_admin_init() {
		...
		add_settings_section(
			...
			__( 'Get your iThemes Brute Force Protection API Key', 'it-l10n-ithemes-security-pro' ),
		...
		*/
		...
		add_settings_field(
			'itsec_ipcheck',
			__( 'Get your iThemes Malware API Key', 'it-l10n-ithemes-security-pro' ),
		...
		*/
		...
		if ( isset( $this->settings['api_key'] ) && isset( $this->settings['api_s'] ) ) {
			...
			add_settings_field(
				...
				__( 'Enable iThemes Brute Force Network Protection', 'it-l10n-ithemes-security-pro' ),
		...
		if ( isset( $input['reset'] ) ) {
			...
			if ( strlen( $raw_email ) > 0 ) {
				...
				if ( $email === false ) {
					...
					$type    = 'error';
					$message = __( 'The email address you used to get an iThemes API key appears to be invalid. Please enter a valid email address', 'it-l10n-ithemes-security-pro' );
				...
				if ( $api_error === true && ! isset( $input['api_key'] ) && ! isset( $input['api_s'] ) ) {
					...
					$type    = 'error';
					$message = __( 'There was an error getting an API key from the IPCheck server. If the problem persists please contact support.', 'it-l10n-ithemes-security-pro' );
	...
	public function settings_field_api_ban() {
		...
		echo '<label for="itsec_ipcheck_api_ban"> ' . __( 'Use the iThemes IPCheck Service to ban IPs reported as a problem by other users in the community.', 'it-l10n-ithemes-security-pro' ) . '</label>';
	...
	private function settings_field_api_key_contents( $module ) {
		...
		if ( $api_key == '' || $api_key == false || $activated == '' || $activated == false ) {
			...
			echo '<input type="text" class="regular-text" id="itsec_ipcheck_email" name="itsec_ipcheck[email-' . $module . ']" value="' . $email . '" placeholder="' . __( 'Enter email and click Save All Changes', 'it-l10n-ithemes-security-pro' ) . '" /><br />';
			echo '<input type="checkbox" id="itsec_ipcheck_optin" name="itsec_ipcheck[optin-' . $module . ']" value="1" checked/>';
			echo '<label for="itsec_ipcheck_optin">' . __( 'Receive email updates about WP Security from iThemes', 'it-l10n-ithemes-security-pro' ) . '</label>';

			echo '<p class="description">' . __( 'Leverage the power of the iThemes Brute Force Protection network to ban IPs hitting your site. ', 'it-l10n-ithemes-security-pro' ) . '</p>';
			...
			echo '<span class="submit"><a class="itsec_reset_ipcheck_api_key button-primary">' . __( 'Reset API Key', 'it-l10n-ithemes-security-pro' ) . '</a></span>';
			echo '<p class="description">' . __( 'Your API key for the iThemes Security community appears to be valid.', 'it-l10n-ithemes-security-pro' ) . '</p>';
			echo '</div>';
